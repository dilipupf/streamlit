...
# Streamlit app code.
# new_title = '<p style="color:Red;">Essentia</p>'
...
if style_select:
    ...
    # dataframe.describe is used to get the mean, std, min, max, etc. of the selected columns in the dataframe
    # This is used to display the statistics of the selected columns in the sidebar
...
# st.write("Selected Tempo range:", tempo_range_val)
...
# st.write("Selected Danceability Range:", danceability)



arousal_range = st.sidebar.slider('🔥 Arousal:', min_value = 1.0, max_value =  9.0, value = [1.0, 9.0])
# st.write("Selected Arousal Range:", arousal_range)

valence_range = st.sidebar.slider('🎶Valence:', min_value = 1.0, max_value =  9.0, value = [1.0, 9.0])
# st.write("Selected Valence Range:", valence_range)
# emoji_html = '&#x1F3B5;'  # musical note emoji

# if st.sidebar.button(HTML(emoji_html)):
#     display(HTML(emoji_html))

# Run the query.
if st.sidebar.button("RUN"):
    st.write('## 🔊 Results')
    mp3s = list(audio_analysis.index)
    # print('mp3s after results', mp3s[0:2])
    ...
    if style_select:
        ...
        # st.write('filtered genre specific songs', df)
        mp3s = df.index
    ...
    # st.write('mp3s after shuffle', mp3s[0:10])
